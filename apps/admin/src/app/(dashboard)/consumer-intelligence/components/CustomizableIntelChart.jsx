...
const SEASON_MAP = {
  12: { name: "Winter", icon: Snowflake, color: "text-blue-400 bg-blue-500/10" },
  ...
  6: { name: "Monsoon", icon: CloudRain, color: "text-cyan-500 bg-cyan-500/10" },
  7: { name: "Monsoon", icon: CloudRain, color: "text-cyan-500 bg-cyan-500/10" },
  8: { name: "Monsoon", icon: CloudRain, color: "text-cyan-500 bg-cyan-500/10" },
  9: { name: "Festive", icon: Flame, color: "text-orange-500 bg-orange-500/10" },
  10: { name: "Festive", icon: Flame, color: "text-orange-500 bg-orange-500/10" },
  11: { name: "Festive", icon: Flame, color: "text-orange-500 bg-orange-500/10" },
};
...
export default function CustomizableIntelChart({ analyticsData }) {
  ...
  const chartItems = useMemo(() => {
    if (dimension === "flavor") {
      const items = flavorPerformance.length > 0
        ...
        : [
            { label: "Classic Himalayan Salt", value: metric === "rating" ? 9.2 : 142, subText: "Avg Rating: 9.2 / 10" },
            { label: "Smoked Chili & Lime", value: metric === "rating" ? 8.8 : 98, subText: "Avg Rating: 8.8 / 10" },
            { label: "Toasted Sesame & Pepper", value: metric === "rating" ? 9.4 : 86, subText: "Avg Rating: 9.4 / 10" },
            { label: "Peri Peri Crunch", value: metric === "rating" ? 8.5 : 74, subText: "Avg Rating: 8.5 / 10" },
            { label: "Cheese Jalapeño", value: metric === "rating" ? 8.9 : 62, subText: "Avg Rating: 8.9 / 10" },
          ];
      return items;
    }
    ...
    if (dimension === "seasonal") {
      if (monthlyTrends.length > 0) {
        return monthlyTrends.map((m) => {
          ...
          return {
            ...
            value: metric === "rating" ? Number(m.avgRating.toFixed(1)) : m.count,
      ...
      // Demo seasonal data
      return [
        { label: "Jan (Winter)", value: metric === "rating" ? 8.1 : 45, subText: "Winter Sampling Campaign" },
        { label: "Feb (Winter)", value: metric === "rating" ? 8.3 : 52, subText: "Winter Sampling Campaign" },
        { label: "Mar (Summer)", value: metric === "rating" ? 8.6 : 78, subText: "Summer Refresh Kickoff" },
        { label: "Apr (Summer)", value: metric === "rating" ? 8.9 : 94, subText: "Peak Summer Demand" },
        { label: "May (Summer)", value: metric === "rating" ? 9.1 : 110, subText: "Summer Promo" },
        { label: "Jun (Monsoon)", value: metric === "rating" ? 8.4 : 68, subText: "Monsoon Snacking" },
        { label: "Jul (Monsoon)", value: metric === "rating" ? 8.5 : 72, subText: "Monsoon Snacking" },
        { label: "Aug (Festive)", value: metric === "rating" ? 9.3 : 135, subText: "Festive Season Start" },
        { label: "Sep (Festive)", value: metric === "rating" ? 9.5 : 160, subText: "Diwali Gift Boxes" },
      ];
    }
    ...
    if (dimension === "channel") {
      ...
      return [
        ...
        { label: "Blinkit / QuickCommerce", value: 48, subText: "Pack QR code" },
      ];
    }

    return [];
  }, [dimension, metric, flavorPerformance, monthlyTrends, ratingDistribution, sourceData]);
  ...
  return (
    <div className="bg-card border border-border/40 p-4 md:p-6 rounded-2xl md:rounded-[2rem] shadow-sm space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-border/20 pb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Analyze flavor demand, seasonality, satisfaction ratings, and channel metrics.
          </p>
        </div>
      ...
      <div className="space-y-4 pt-2">
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span className="font-semibold text-primary">
            Metric: {metric === "rating" ? "Average Rating" : "Total Feedback Volume"}
          </span>
        </div>
            ...
            return (
              <div key={idx} className="group space-y-1.5">
                ...
                <div className="h-3 w-full bg-muted/30 rounded-full overflow-hidden p-0.5 border border-border/30">
                  <div
                    className="h-full bg-gradient-to-r from-primary/70 to-primary rounded-full transition-all duration-700 ease-out group-hover:from-primary group-hover:to-amber-400"
