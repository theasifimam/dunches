...
function AuthStep({ onSuccess }) {
  ...
  const [step, setStep] = useState("phone"); // phone | password | otp | register
  ...
  useEffect(() => {
    if (step === "otp" && otp.join("").length === 4) setStep("register");
  }, [otp, step]);
    ...
    >
      <div className="text-center mb-8">
        ...
        <h2 className="text-3xl font-light font-serif tracking-tighter mb-1 lowercase">
          ...
          {step === "register" && "one last step."}
        </h2>
        <p className="text-[10px] text-foreground/40 font-bold uppercase tracking-widest">
          {step === "phone" && "We'll check if you're already a member"}
          {step === "password" && `Password for ${maskedPhone}`}
          {step === "otp" && `OTP sent to ${maskedPhone}`}
          {step === "register" && "Set your name and password"}
        </p>
      </div>

      {error && (
        <div className="mb-4 text-[10px] font-black text-red-500 bg-red-500/5 border border-red-500/10 p-3 rounded-2xl tracking-widest uppercase text-center">
          {error}
        </div>
      )}
      ...
      {step === "password" && (
        <form onSubmit={handlePasswordLogin} className="space-y-4">
          ...
          <button
            ...
            className="w-full text-center text-[9px] font-black text-primary/60 hover:text-primary uppercase tracking-widest flex items-center justify-center gap-1 mt-4"
          >
      ...
      {/* Register (new user — set name + password) */}
      {step === "register" && (
...
function PaymentStep({ user, address, cartItems, cartTotal, onSuccess }) {
  ...
  return (
    ...
    >
      ...
      {/* Order Summary */}
      <div className="p-6 rounded-2xl bg-foreground/[0.02] border border-border/50 mb-6 space-y-4">
